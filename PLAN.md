...
### 7단계: VOD 직접 업로드
1.  **API 엔드포인트 구현:** `POST /api/vods/upload` 라우트를 생성하고, `multer`를 사용한 파일 업로드 미들웨어를 설정합니다.
2.  **업로드 유스케이스 구현:** `UploadVod.usecase.ts`를 생성하여, 업로드된 파일을 기존 VOD 처리 큐에 추가하는 로직을 구현합니다.
3.  **의존성 주입:** `app.ts`의 Composition Root에서 `UploadVodUseCase`를 인스턴스화하고 `VodController`에 주입합니다.
4.  **테스트 작성:** `UploadVod.usecase`에 대한 유닛 테스트와 업로드 API 엔드포인트에 대한 E2E 테스트를 작성합니다.

### 8단계: 코드 리팩토링 및 개선
1.  **`Result` 패턴 도입**: 예외(Exception)를 던지는 대신 `Result` 객체를 반환하도록 하여, 오류 처리를 명시적이고 안정적으로 만듭니다.
2.  **설정 파일 중앙 관리**: 하드코딩된 설정값(JWT 시크릿 키 등)을 `.env`와 중앙 설정 파일을 통해 관리하여 보안과 유지보수성을 향상시킵니다.
3.  **응답(Response) DTO 도입**: 컨트롤러가 도메인 엔티티에 직접 의존하지 않도록, 유스케이스가 외부에 노출할 데이터만 담은 DTO를 반환하도록 변경합니다.

codex resume 019ada16-1f95-7dd0-b065-6898a92adb3c